...
using Visitors_Registration_System.Data.Repositories;
using Visitors_Registration_System.Helpers;

namespace Visitors_Registration_System
{
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            .AddJwtBearer(x =>
            {
                x.Events = new JwtBearerEvents
                {
                    ...
                    {
                        var userService = context.HttpContext.RequestServices.GetRequiredService<IAdmin>();
                ...
                x.Events = new JwtBearerEvents
                {
                    OnTokenValidated = context =>
                    {
                        var userService = context.HttpContext.RequestServices.GetRequiredService<IVisitors>();
                        var userId = Guid.Parse(context.Principal.Identity.Name);
                        var user = userService.GetById(userId);
                        if (user == null)
                        {
